---
title: "Debian 12 使用 Docker 部署 MySQL 8"
date: 2025-09-24
draft: false
tags: ["MySQL", "Docker", "Debian", "数据库部署"]
categories: ["部署脚本"]
description: "在 Debian 12 上通过 Docker 快速部署 MySQL 8 的完整脚本示例，包含配置文件、管理脚本和远程用户创建。"
---

> 本脚本适用于 Debian 12 环境，自动完成 MySQL 8 的 Docker 部署与基础配置。
...
echo -e "${YELLOW}[1/4] 拉取 MySQL 8 镜像...${NC}"
docker pull mysql:8

echo -e "${YELLOW}[2/4] 创建数据与配置目录...${NC}"
mkdir -p ${MYSQL_DATA_DIR}
mkdir -p /etc/mysql-docker
cat > /etc/mysql-docker/my.cnf << 'MYSQL_EOF'
[mysqld]
...
docker run -d \
  ...
  -v /etc/mysql-docker/my.cnf:/etc/mysql/conf.d/my.cnf \
...
if docker ps | grep -q ${MYSQL_CONTAINER_NAME}; then
    echo -e "${GREEN}MySQL 8 部署成功！${NC}"
    echo -e "${GREEN}容器名称: ${MYSQL_CONTAINER_NAME}${NC}"
    echo -e "${GREEN}端口: ${MYSQL_PORT}${NC}"
    echo -e "${GREEN}数据目录: ${MYSQL_DATA_DIR}${NC}"
    echo -e "${GREEN}配置文件: /etc/mysql-docker/my.cnf${NC}"
    echo -e "${GREEN}Root 密码: ${MYSQL_ROOT_PASSWORD}${NC}"

    # 创建管理脚本
    cat > /usr/local/bin/mysql-docker << 'SCRIPT_EOF'
#!/bin/bash
case "$1" in
    start) docker start mysql8 ;;
    stop) docker stop mysql8 ;;
    restart) docker restart mysql8 ;;
    status) docker ps -a | grep mysql8 ;;
    logs) docker logs -f mysql8 ;;
    shell) docker exec -it mysql8 bash ;;
    mysql) docker exec -it mysql8 mysql -u root -p ;;
    *) echo "用法: $0 {start|stop|restart|status|logs|shell|mysql}"; exit 1 ;;
esac
SCRIPT_EOF
    chmod +x /usr/local/bin/mysql-docker
    echo -e "${GREEN}管理命令已创建: mysql-docker {start|stop|restart|status|logs|shell|mysql}${NC}"
